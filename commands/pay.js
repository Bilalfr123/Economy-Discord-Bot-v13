...
// slash:'both',
    ...
    callback:async({message,args,client})=>{
        if (!args.length) {
            message.channel.send("You need to mention a player to give them coins");
            return
        } 
        const amount = args[1];
        ...
        if (amount > profileData.coins){
            message.channel.send(`You don't have that amount of coins to pay them`)
            return
        }  
          ...
          await client.users.fetch(target.id).then((target)=>{
            target.send(
               `you have been given ${amount} of coins! by user`
        )
            })
           message.channel.send(`you gave them ${amount} of coins!`);
        } catch (err) {
          console.log(err);
        }
            }
